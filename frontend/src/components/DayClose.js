// src/components/DayClose.jsx  (Z Report — Day Close)
import React, { useState } from "react";
import PropTypes from "prop-types";
import api from "../api/axios";
import { printInIframe, buildThermalHtml } from "../utils/thermalPrint";

const STORE = { name: "POS STORE", phone: "+971-55-123-4567", trn: "100123456700003", address: "" };

const fmt = (n) =>
  Number(n || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 });

export default function DayClose({ open, onClose }) {
  ...
  const fetchZReport = async () => {
    ...
    // optional “day close”; never throws & asks logger to stay quiet
    await api.post(
      "/reports/day-close",
      { date: todayStr },
      { validateStatus: () => true, headers: { "x-skip-error-log": "1" } }
    ).catch(() => {});

    const { data } = await api.get("/reports/dayz", {
      params: { from: todayStr, to: todayStr },
      // also silence any logging interceptors for this read
      headers: { "x-skip-error-log": "1" },
      validateStatus: () => true,
    });
  ...
  const buildReceiptHtml = (p) => {
    ...
    return `
      ...
      <div class="center"><small>POS Z REPORT (Day Close)</small></div>
  ...
  const savePdf = async (isoDate) => {
    const res = await api.get("/reports/z/pdf", {
      params: { date: isoDate },
      responseType: "blob",
      headers: { Accept: "application/pdf", "x-skip-error-log": "1" },
      validateStatus: () => true,
    });
    downloadBlob(res.data, `ZReport-${isoDate}.pdf`);
  };

  const saveHtmlFallback = (payload, isoDate) => {
    const body = buildReceiptHtml(payload);
    const html = buildThermalHtml("Z Report", body);
    const blob = new Blob([html], { type: "text/html;charset=utf-8" });
    downloadBlob(blob, `ZReport-${isoDate}.html`);
  };

  const handleAction = async (action) => {
    try {
      setBusy(true);
      const { todayStr, payload } = await fetchZReport();

      if (action === "print") {
        const body = buildReceiptHtml(payload);
        const html = buildThermalHtml("Z Report", body);
        printInIframe(html, "Z Report");
        onClose?.();
        return;
      }
      if (action === "save") {
        try { await savePdf(todayStr); }
        catch { saveHtmlFallback(payload, todayStr); }
        onClose?.();
      }
    } catch (e) {
      alert(errText(e));
  ...
  return (
    <div className="xreport-modal-backdrop">
      <div className="xreport-modal" role="dialog" aria-modal="true" aria-labelledby="z-title">
        <div className="xreport-header">
          <span id="z-title">POS</span>
          <button className="close-btn" onClick={onClose} aria-label="Close">✕</button>
        </div>

        <div className="xreport-body">
          <h3 className="title">PRINT Z REPORT (DAY CLOSE)?</h3>
        </div>
